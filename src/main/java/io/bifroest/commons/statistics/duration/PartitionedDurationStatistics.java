package io.bifroest.commons.statistics.duration;
...
import io.bifroest.commons.statistics.aggregation.WindowMaxAggregation;
import io.bifroest.commons.statistics.calllog.CallLog;
import io.bifroest.commons.statistics.jmx.TabularDataFromValueAggregationMap;
...
public class PartitionedDurationStatistics implements PartitionedDurationStatisticsMBean {
    ...
    private static final Logger log = LogService.getLogger(PartitionedDurationStatistics.class);
    ...
    public PartitionedDurationStatistics( int windowSize, int callLogSize, int expectedPartitionCount ) {
        ...
        numberOfCalls = new HashMap<>( expectedPartitionCount );
        averages = new HashMap<>( expectedPartitionCount );
        averagesAcrossLast = new HashMap<>( expectedPartitionCount );
        maxes = new HashMap<>( expectedPartitionCount );
