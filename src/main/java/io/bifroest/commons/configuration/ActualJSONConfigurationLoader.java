...
import io.bifroest.commons.util.json.IgnoreCommentsReader;
import io.bifroest.commons.util.json.JSONUtils;

public class ActualJSONConfigurationLoader< E extends EnvironmentWithConfigPath & EnvironmentWithMutableJSONConfiguration > implements JSONConfigurationLoader {
    ...
    private final Map<Path, Optional<String>> parseErrors = new HashMap<>();
    ...
    private class JSONFileVisitor extends SimpleFileVisitor<Path> {
        ...
        public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
            ...
            if (Files.isSymbolicLink(file)) {
                ...
                try {
                    Files.walkFileTree(target, EnumSet.of(FileVisitOption.FOLLOW_LINKS), maxDepth(), this);
                } catch( NoSuchFileException e ) {
                    log.warn( "NoSuchFileException while walking " + target + ". Broken symlink?" );
                }
